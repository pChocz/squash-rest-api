...
class MagicLoginLinkEmailService {
    ...
    void pushEmailToQueue(final String email, final String name, final Locale locale, final String passwordResetLink) {
        ...
        model.put("primaryContent", messageSource.getMessage("message.magicLoginLink.youHaveRequested", null, locale));

        model.put("clickMessage", messageSource.getMessage("message.magicLoginLink.buttonText", null, locale));
