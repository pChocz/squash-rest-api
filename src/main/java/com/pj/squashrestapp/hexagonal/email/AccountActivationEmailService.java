...
class AccountActivationEmailService {
    ...
    void sendEmail(final String email, final String name, final Locale locale, final String activationLink) {
        ...
        model.put("link", activationLink);

        // static
        model.put("hiMessage", messageSource.getMessage("message.util.hi", new Object[] {name}, locale));

        model.put(
                "primaryContent", messageSource.getMessage("message.activation.youHaveCreatedAnAccount", null, locale));

        model.put("clickMessage", messageSource.getMessage("message.activation.buttonText", null, locale));
