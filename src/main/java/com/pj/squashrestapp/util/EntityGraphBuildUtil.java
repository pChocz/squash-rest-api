...
/**
 * Entity Graph Reconstructor for the {@link SetResult} class that allows to build entire structure
 * above, up to league entity.
 */
@UtilityClass
public class EntityGraphBuildUtil {

  public League reconstructLeague(final List<SetResult> setResults, final Long leagueId) {
    final EntityGraphBuilder entityGraphBuilder =
    ...
    return entityGraphBuilder.getEntityContext().getObject(leagueClassId);
  }
  ...
  public Season reconstructSeason(final List<SetResult> setResults, final Long seasonId) {
    ...
    return entityGraphBuilder.getEntityContext().getObject(seasonClassId);
  }
  ...
  public Round reconstructRound(final List<SetResult> setResults, final Long roundId) {
    ...
    return entityGraphBuilder.getEntityContext().getObject(roundClassId);
  }

  public ArrayListMultimap<String, Integer> reconstructXpPointsForRound(
      final List<XpPointsForRound> allPointsForRounds, final List<XpPointsForPlace> allPoints) {
    final EntityGraphBuilder entityGraphBuilder =
        new EntityGraphBuilder(
                new EntityVisitor[] {
    ...
    for (final XpPointsForRound xpPointsForRound : allPointsForRounds) {
      final ClassId<XpPointsForRound> xpPointsForRoundClassId =
          new ClassId<>(XpPointsForRound.class, xpPointsForRound.getId());

      final XpPointsForRound xpPointsForRoundReconstructed =
          entityGraphBuilder.getEntityContext().getObject(xpPointsForRoundClassId);
      ...
      for (final XpPointsForRoundGroup xpPointsForRoundGroup :
          xpPointsForRoundReconstructed.getXpPointsForRoundGroups()) {
        for (final XpPointsForPlace xpPointsForPlaces :
  ...
  public List<XpPointsForRound> reconstructXpPointsList(
      final List<XpPointsForRound> allPointsForRounds, final List<XpPointsForPlace> allPoints) {
    final EntityGraphBuilder entityGraphBuilder =
        new EntityGraphBuilder(
                new EntityVisitor[] {
    ...
    final List<XpPointsForRound> xpPointsForRoundList =
        allPointsForRounds.stream()
            .map(
                xpPointsForRound -> new ClassId<>(XpPointsForRound.class, xpPointsForRound.getId()))
            .map(
                xpPointsForRoundClassId ->
                    entityGraphBuilder.getEntityContext().getObject(xpPointsForRoundClassId))
