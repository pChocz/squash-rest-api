...
class LogExtractService {
    ...
    Set<LogBucket> extractLogBuckets(
            final Criteria criteria, final Date start, final Date stop, final int numberOfBuckets) {
        ...
        final BucketOperation bucketOperation = Aggregation.bucket(LogConstants.FIELD_TIMESTAMP)
                .withBoundaries((Object[]) bucketsBoundaries)
                .andOutputCount()
                .as(LogConstants.FIELD_AGGREGATE_SUM_COUNT);
    ...
    List<LogAggregateByMethod> logAggregateByMethod(final Criteria criteria) {
        final MatchOperation match = Aggregation.match(new Criteria()
                .andOperator(
                        criteria, Criteria.where(LogConstants.FIELD_METHOD_NAME).exists(true)));

        final GroupOperation group = Aggregation.group(LogConstants.FIELD_METHOD_NAME)
                .count()
                .as(LogConstants.FIELD_AGGREGATE_SUM_COUNT)
                .sum(LogConstants.FIELD_DURATION)
                .as(LogConstants.FIELD_AGGREGATE_SUM_DURATION)
                .min(LogConstants.FIELD_DURATION)
                .as(LogConstants.FIELD_AGGREGATE_MIN_DURATION)
                .max(LogConstants.FIELD_DURATION)
                .as(LogConstants.FIELD_AGGREGATE_MAX_DURATION)
                .avg(LogConstants.FIELD_DURATION)
                .as(LogConstants.FIELD_AGGREGATE_AVG_DURATION)
                .stdDevSamp(LogConstants.FIELD_DURATION)
                .as(LogConstants.FIELD_AGGREGATE_DEV_DURATION)
                .sum(LogConstants.FIELD_QUERY_COUNT)
                .as(LogConstants.FIELD_AGGREGATE_SUM_QUERY_COUNT);
    ...
    List<LogAggregateByUser> logAggregateByUser(final Criteria criteria) {
        final MatchOperation match = Aggregation.match(new Criteria()
                .andOperator(
                        criteria, Criteria.where(LogConstants.FIELD_USERNAME).exists(true)));
    ...
    LogEntriesPaginated extractLogs(final Criteria criteria, final Pageable pageable) {
        ...
        Page<LogEntry> page = PageableExecutionUtils.getPage(
                list,
                pageable,
                () -> mongoTemplate.count(query.limit(-1).skip(-1), LogEntry.class, LogConstants.COLLECTION_NAME));
    ...
    public LogSummary buildLogSummary(
        ...
        logSummary.setLogBuckets(
                new ArrayList<>(extractLogBuckets(criteria, start.orElseThrow(), stop.orElseThrow(), numberOfBuckets)));
