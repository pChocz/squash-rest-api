...
public class FakePlayersSelectorRoundGroupAware {
  ...
  public ArrayListMultimap<Integer, Player> assign(
    ...
    final int numberOfGroupsInNewRound = switch (numberOfRoundPlayers) {
      case 4, 5, 6, 7 -> 1;
    ...
    for (int i=1; i <= numberOfGroupsInPreviousRound; i++) {
      final RoundGroupScoreboard roundGroupScoreboard = previousRoundScoreboard.getRoundGroupScoreboards().get(i - 1);
    ...
    for (int groupNumber = 1; groupNumber <= numberOfGroupsInPreviousRound; groupNumber++) {
      final RoundGroupScoreboard roundGroupScoreboard = previousRoundScoreboard.getRoundGroupScoreboards().get(groupNumber - 1);
